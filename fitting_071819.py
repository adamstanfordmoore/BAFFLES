...
def vs_age_fits(bv,cluster_ages,rhk,scatter,metal,li_scatter_fit=None,ca_scatter=None,fit_gauss=None):
    ...
    if (metal == 'calcium'):
        ...
        #fit_transform = lambda x: np.hstack([x**2])
        #x_new = fit_transform(np.log10(cluster_ages).reshape(-1,1))
        #model = LinearRegression()
        #model.fit(x_new,rhk)
        #def skm(age):
        #    print model.coef_
        #    return model.predict(fit_transform(np.array(age).reshape(-1,1)))
        
        #metal_fit = skm
        
        #rhk = savgol_filter(rhk, 7, 3) # window size 51, polynomial order 3
        #metal_fit = interpolate.interp1d(np.log10(cluster_ages),rhk,fill_value='extrapolate')
        
        cluster_ages = np.log10(cluster_ages)
        if (True):
            bv_cut = [0.5,1.2,1.6]#[.85,1.3]
            segs = [2,3,2,1]#[4,3,2]
            ...
            #metal_fit = simple_pwise_fit(cluster_ages,rhk,segs[bisect.bisect_left(bv_cut,bv)])
            #metal_fit = pwise_fit(cluster_ages,rhk,5,x_method='even',bin_size=1,min_length= 0.3)[0]
        
        elif (bv < .61):
            #linear_fit = poly_fit(np.log10(cluster_ages[0:2]),rhk[0:2],1)
            #node = np.log10(cluster_ages[1])
            metal_fit = constrained_poly_fit(cluster_ages,rhk,cluster_ages[0],rhk[0])
            #metal_fit = lambda age: np.where(age < node,linear_fit(age),constr_poly_fit(age))
            mu_lbl = 'Constrained Polynomial'
        elif (bv < 0.84):
            f = make_half_n_half(cluster_ages,rhk)
            popt,pcov = curve_fit(f,cluster_ages,rhk,p0=[1.5])
            print popt
            metal_fit = lambda age : f(age,*popt)
            mu_lbl = 'linear/poly'
            """
            f = constr_inverse_x(cluster_ages[1],rhk[1])
            popt,pcov = curve_fit(f,cluster_ages[1:],rhk[1:],p0=[3,1,-3])
            print popt
            cutoff = -popt[2]/popt[1]
            def m_fit(age):
                fit = lambda age: constr_inverse_x(cluster_ages[0],rhk[0])(age,*popt)
                return np.where(age < cutoff,fit(age),-np.inf)

            linear_fit = poly_fit(cluster_ages[0:2],rhk[0:2],1)
            node = cluster_ages[1]
            metal_fit = lambda age: np.where(age < node,linear_fit(age),m_fit(age))
            mu_lbl = '1/x fit'
            """
        #elif (bv < 1):
        #    metal_fit = pwise_fit(cluster_ages,rhk,2,x_method='free')[0]

        elif (bv <= 1.356): #soderblom 4000K boundary
            #bounds = ([-np.inf,-np.inf,-np.inf],np.inf)
            f = constr_inverse_x(cluster_ages[0],rhk[0])
            popt,pcov = curve_fit(f,cluster_ages,rhk,p0=[ 2.31,2.575,-8.987])
            cutoff = -popt[2]/popt[1]
            def m_fit(age):
                fit = lambda age: constr_inverse_x(cluster_ages[0],rhk[0])(age,*popt)
                return np.where(age < cutoff,fit(age),-np.inf)
            ...
            linear_fit1 = poly_fit(cluster_ages[0:2],rhk[0:2],1)
            node = cluster_ages[1]
            def make_fixed_linear(x1,y1):
                def constr_line(x,m):
                    b = y1 - m*x1
                    return m*x + b
                return constr_line
            f = make_fixed_linear(cluster_ages[1],rhk[1])
            popt,pcov = curve_fit(f,cluster_ages[1:],rhk[1:])
            linear_fit2 = lambda x : f(np.array(x),*popt)
            #constr_poly = constrained_poly_fit(cluster_ages[1:],rhk[1:],\
            #        scatter[1:],node)
            metal_fit = lambda age: np.where(age < node,linear_fit1(age),linear_fit2(age))
            #metal_fit = piecewise([cluster_ages[0],2.2,3],[rhk[0],2.1,.5])
            mu_lbl = 'Piecewise'
    def mu(age):
