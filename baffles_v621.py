...
def baffles_age(bv,rhk=None,li=None,bv_err=BV_UNCERTAINTY,upperLim=False,maxAge=const.GALAXY_AGE,fileName='baffles',pdfPage=None,showPlots=True,noPlots=False,savePostAsText=False):
    ...
    if (rhk):
        ...
        p = baf.get_posterior(bv,rhk,pdfPage,showPlots,bv_uncertainty=bv_err,isUpperLim=upperLim,maxAge=maxAge,mamajekAge=getMamaAge(rhk))
        if (savePostAsText):
...
class age_estimator:
    ...
    def get_posterior(self,bv,metallicity,pdfPage=None,showPlot=False,bv_uncertainty=BV_UNCERTAINTY,isUpperLim=False,maxAge=None,givenAge=None,givenErr = None,mamajekAge=None,title=None,logPlot = False):
        if not maxAge and isUpperLim: maxAge = self.const.GALAXY_AGE
        array = self.age_dist_uncertainty(bv,bv_uncertainty,metallicity,isUpperLim,maxAge)
        prob.normalize(self.const.AGE,array)
        p_struct = posterior()
        p_struct.array = array
        p_struct.stats = prob.stats(self.const.AGE,array,isUpperLim)
        p_struct.upperLim = isUpperLim
        if (showPlot or pdfPage):
            if (title == None):
                title = 'Posterior Age Probabilty for (B-V)o = '+'%.2f' % bv +', ' + self.metal + ' = %.2f' % metallicity
            my_plot.posterior(self.const.AGE, p_struct.array, p_struct.stats,title,pdfPage,showPlot,givenAge=givenAge,givenErr=givenErr, mamajekAge=mamajekAge,logPlot=logPlot) 
        return p_struct
    
    def posterior_product(self,bv_arr,metallicity_arr,upperLim_arr=None,maxAge_arr = None, pdfPage=None,showPlot=False,showStars=False,title=None,givenAge=None,givenErr = None,bv_errs=None):
        if (not bv_errs):
            bv_errs = [BV_UNCERTAINTY]*len(bv_arr)
        if not upperLim_arr:
            upperLim_arr = [False]*len(metallicity_arr)
        if not maxAge_arr:
            maxAge_arr = [self.const.GALAXY_AGE]*len(metallicity_arr)
        ln_prob = np.zeros(len(self.const.AGE))
        star_post = []
        for i in range(len(bv_arr)):
                y = self.age_dist_uncertainty(bv_arr[i],bv_errs[i],metallicity_arr[i],upperLim_arr[i],maxAge_arr[i])
                ln_prob += np.log(y)
                if (showStars):
    ...
    def age_dist_uncertainty(self,bv,sig,rhk,isUpperLim,maxAge):
        ...
        for i in range(len(self.const.BV)):
            if (gauss[i] > np.log(prob.FIVE_SIGMAS)):
                arr += np.exp(gauss[i] + self.log_likelihood(i,rhk,isUpperLim,maxAge) - a)
    ...
    def log_likelihood(self,b,rhk,isUpperLim,maxAge):
    ...
    def make_grids(self,bv_li,fits,medianSavefile=None,sigmaSavefile=None,setAsDefaults=False, omit_cluster=None,upper_lim=None):
        ...
        for bv in self.const.BV:
            ...
            if (self.metal == 'calcium'):
                ...
                scatter_fit = my_fits.fit_gaussian(np.log10(CLUSTER_AGES),scatter)
