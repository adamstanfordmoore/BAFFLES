...
class age_estimator:
    ...
    #changes constant file to have this grid file (in baffles/grids) as the default
    def set_default_grids(self,grid_median):
        from baffles.paths import PACKAGE_DIR
        name = os.path.basename(grid_median)
        if not name.endswith('.npy'):
            name += '.npy'
        filename = os.path.join(PACKAGE_DIR, 'li_constants.py' if self.metal=='lithium' else 'ca_constants.py')
        lines = open(filename,'r').readlines()
        for l in range(len(lines)):
            if (lines[l][:14] == 'DEFAULT_MEDIAN'):
                lines[l] = 'DEFAULT_MEDIAN_GRID = join(GRID_DIR, "' + name + '")\n'
        out = open(filename,'w')
